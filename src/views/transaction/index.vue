  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="充值支付凭证图" show-overflow-tooltip width="150" align="center">
        <template slot-scope="scope">
          <!-- <span>{{ scope.row.screenshot }}</span> -->
          <el-image
            v-if="scope.row.type === '000' && scope.row.recharge_type == 1"
...
export default {
  ...
  computed: {
    ...mapState({
      transactionList: state => state.stock.transactionList,
  ...
  methods: {
    ...
    getTransaction() {
      ...
      getTransactionApi(send_).then(res => {
        ...
        if (res.success && res.code === 1000) {
          this.$store.commit('stock/SET_TRANSACTION_LIST', res.data)
        }
    ...
    doTran(row, action) {
      ...
      if (this.action === 'accept') {
        this.showText = `通过` 
        // this.showText = `您确认要通过 => 账号：${row.user.name} 手机：${row.user.phone} 申请的订单吗? 金额：${row.amount} ` 
      }

      if (this.action === 'deny') {
        this.showText = `拒绝` 
        // this.showText = `您确认要拒绝处理 => 账号：${row.user.name} 手机：${row.user.phone} 申请的订单吗? 金额：${row.amount} ` 
      }
