<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="账号" show-overflow-tooltip width="130" align="center">
        <template slot-scope="scope">
          {{ scope.row.user == null ? '' : scope.row.user.name }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="订单状态" show-overflow-tooltip width="110" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="申请日期" show-overflow-tooltip width="170" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.createdAt | dateFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column label="收款人姓名" show-overflow-tooltip width="150" align="center">
        <template slot-scope="scope">
          <div v-if="scope.row.user.UserBanks[0]">
            <span>{{ scope.row.user.UserBanks[0]['holder_name'] }}</span>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="收款人账号" show-overflow-tooltip width="150" align="center">
        <template slot-scope="scope">
          <div v-if="scope.row.user.UserBanks[0]">
            <span>{{ scope.row.user.UserBanks[0].account }}</span>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="收款银行名称" show-overflow-tooltip width="150" align="center">
        <template slot-scope="scope">
          <div v-if="scope.row.user.UserBanks[0]">
            <span>{{ scope.row.user.UserBanks[0].bank_name }}</span>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="收款信息备注" show-overflow-tooltip width="150" align="center">
        <template slot-scope="scope">
          <div v-if="scope.row.user.UserBanks[0]">
            <span>{{ scope.row.user.UserBanks[0].remark }}</span>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="订单备注" show-overflow-tooltip min-width="150" align="left">
        <template slot-scope="scope">
...
export default {
  ...
  computed: {
    ...mapState({
      transactionList: state => state.stock.transactionListWithdraw,
      tranFilter: state => state.stock.transactionFilter,
      conws: state => state.wsuse.connected
    }),
    ...mapGetters(['imageBase', 'token'])
  },
  mounted() {
    ...
    getTransaction() {
      ...
      getTransactionApi(send_).then(res => {
        ...
        if (res.success && res.code === 1000) {
          this.$store.commit('stock/SET_TRANSACTION_LIST_W', res.data)
        }
